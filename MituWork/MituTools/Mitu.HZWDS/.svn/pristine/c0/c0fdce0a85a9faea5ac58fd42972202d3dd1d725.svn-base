...
(function ($) {
    ...
    Toolkit.base.suggest = function (input, options) {
        ...
        var postdata = { "RegName": $input.attr("data-regName"), "Text": "" };
        ...
        function suggest() {
            ...
            if (q.length >= options.minchars) {
                ...
                postdata.Text = q;
                //postdata['SELECT'][0]['VALUE'] = q;
                //if (options.refFields) {
                //	postdata['REF'] = [];
                //	for (var item in options.refFields) {
                //		var itemValue = Toolkit.data.getElementValue(options.refFields[item]);
                //		postdata['REF'].push({Field:item,RefValue:itemValue});
                //	}
                //}
                $.ajax({
                    type: 'post',
                    ...
                    success: function (data) {
                        hideSuggest();
                        if (data && data['EasySearch']) {
                            displayItems(data['EasySearch']);
                        }
                    }
                });
            } else {
        ...
        function displayItems(items) {
            ...
            var htmlstr = '';
            var htmlTemplate = '<li class="list-group-item" data-id="{Value}" data-text="{Name}"><span>{Name}</span></li>';

...
namespace YJC.Toolkit.Sys
{
    public static class NetUtil
    {
        ...
        public static WebResponse HttpGet(Uri uri, string contentType)
        {
            ...
            Trace.WriteLine(uri.ToString(), "Toolkit HttpGet");
        ...
        public static WebResponse FormUploadFile(Uri url, string controlName, string fileName, string videoDecription = null)
        {
            TkDebug.AssertArgumentNullOrEmpty(fileName, "fileName", null);

            byte[] data = File.ReadAllBytes(fileName);

            return FormUploadFile(url, controlName, fileName, data, videoDecription);
        }
        ...
        public static WebResponse FormUploadFile(Uri url, string controlName,
            string fileName, byte[] fileData, string videoDecription = null)
        {
            ...
            byte[] formData = GetMultipartFormData(formDataBoundary, controlName, fileName, fileData, videoDecription);
        ...
        //
        // description： 视频素材的描述信息，内容格式为JSON,
        // 如{"title":VIDEO_TITLE,"introduction":INTRODUCTION}
        //
            ...
            string fileName, byte[] fileData, string description)
        {
            ...
            using (formDataStream)
            {
                ...
                string header = string.Format(ObjectUtil.SysCulture,
                    ...
                    new object[] { boundary, controlName, Path.GetFileName(fileName), GetContentType(fileName) });
                ...
                if (description != null)
                {
                    ...
                    formDataStream.Write(encoding.GetBytes(header), 0, encoding.GetByteCount(header));
                    byte[] desData = encoding.GetBytes(description);
